...
import { buildReferenceRenderPrompt } from "@/lib/ai/prompts";
...
export async function POST(request: Request) {
  ...
  const limit = rateLimit(`render-hero-ref:${ip}`, 5, 60000);
  if (!limit.success) {
  ...
  try {
    ...
    // Fetch project to get hero image URL
    const project = await projectsTable.find(projectId);
    const heroImageUrl = project.get("Hero Image URL") as string;

    if (!heroImageUrl) {
      return NextResponse.json(
        { error: "Hero image must be approved first. Generate or upload a hero shot." },
        { status: 400 }
      );
    }

    // Download hero image to base64
    const heroResponse = await fetch(heroImageUrl);
    if (!heroResponse.ok) {
      return NextResponse.json(
        { error: "Failed to download hero image" },
        { status: 500 }
      );
    }
    const heroBuffer = await heroResponse.arrayBuffer();
    const heroBase64 = Buffer.from(heroBuffer).toString("base64");

    // Fetch all raw captures EXCEPT three_quarter (already the hero)
    ...
    // Skip views that already have hero-reference renders
    const heroRefViews = allViewRecords.filter(
      (r) => r.get("Is Hero Reference") === true && r.get("View Name") !== "three_quarter"
    );
    const alreadyRendered = new Set(heroRefViews.map((r) => r.get("View Name") as string));
    const viewsToRender = rawViews.filter((r) => !alreadyRendered.has(r.get("View Name") as string));

    if (viewsToRender.length === 0) {
      return NextResponse.json(
        { error: "All views already have hero-reference renders." },
        { status: 400 }
      );
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // Helper: call Gemini with hero reference + capture image
        async function callGeminiWithReference(model: string, captureBase64: string, viewName: string) {
          const referencePrompt = buildReferenceRenderPrompt(viewName);
            ...
            contents: [
              {
                ...
                parts: [
                  ...
                  { inlineData: { mimeType: "image/png", data: heroBase64 } },
                  { inlineData: { mimeType: "image/png", data: captureBase64 } },
                ],
              },
            ],
            config: {
        ...
        async function renderView(view: { viewName: string; imageUrl: string }) {
          ...
          if (!imageResponse.ok) {
            return { viewName: view.viewName, error: "Failed to download capture image" };
          }
          const imageBuffer = await imageResponse.arrayBuffer();
          const captureBase64 = Buffer.from(imageBuffer).toString("base64");

          // Try active model, fallback on overload (503)
          let response;
          try {
            response = await callGeminiWithReference(activeModel, captureBase64, view.viewName);
          } catch (primaryErr) {
          ...
          const blob = await put(
            `renders/${projectId}/${view.viewName}-hero-ref.png`,
            renderedBuffer,
        ...
        // Process views in parallel batches of 3 (2 images per call = heavier)
        const BATCH_SIZE = 3;
        for (let batchStart = 0; batchStart < views.length; batchStart += BATCH_SIZE) {
          ...
          sendEvent("progress", {
            ...
            message: `Rendering ${batch.length} views with hero reference...`,
          });
          ...
          for (let j = 0; j < results.length; j++) {
            ...
            if (result.status === "fulfilled") {
              ...
              console.error(`Hero-ref render ${view.viewName} failed:`, errMsg);
              ...
              if (isQuotaError) {
                ...
                errorSent = true;
                sendEvent("error", { message: "Gemini API key is invalid or lacks permission for image generation." });
